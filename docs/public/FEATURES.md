...
6. [Auto-Type — answers typed into your editor (Max)](#6-auto-type--answers-typed-into-your-editor-max)
7. [Train Model — pre-research the role (Max)](#7-train-model--pre-research-the-role-max)
...
11. [Web-search-aware answers (Max)](#11-web-search-aware-answers-max)
12. [Conversation history](#12-conversation-history)
13. [Reasoning-effort knob (Max)](#13-reasoning-effort-knob-max)
...
## 6. Auto-Type — answers typed into your editor (Max)

Available on the Max plan. Click the Auto-Type icon, then click into your code editor (the IDE, online judge, or coding sandbox). The AI's answer types into the editor at a realistic human cadence with natural pauses and corrections.
...
## 7. Train Model — pre-research the role (Max)

Available on the Max plan. Before your interview, click **Train Model** in the chat sidebar. The app reads your résumé and the job description, then prepares a memory pack covering:
...
- **Gemini** (Google) — fast, free-tier default. Strong on general questions; lighter touch on code.
- **GPT** (OpenAI) — strong general-purpose model. Available on Basic+.
- **Grok** (xAI) — fast inference. Available on Basic+.
- **Groq** (open-weight on Groq's fast inference) — lowest latency. Available on Basic+.
- **Claude** (Anthropic) — strongest on long-form reasoning and code. Available on Max only, includes web search.
...
History lives on your machine in a local database; a mirror copy is on our servers so you can pick up where you left off on a fresh install or new device. Cleared 180 days after subscription cancellation; deleted within 30 days if you delete your account.

## 13. Reasoning-effort knob (Max)

For Max-tier users on Claude. The reasoning slider in the chat header has three notches:
...
- **Razorpay** — INR, UPI, India-issued cards, NetBanking.

You can't pay one company's subscription with the other provider's account. The picker shows the right provider for your region automatically.
...
| Minica support chatbot | Scripted FAQ | Full AI chat | Full AI chat | Full AI chat |
