...
// This file pins STRICT mode — the end state, once every install sends a
// code. It must be set explicitly rather than inherited from the default,
// because the default is deliberately OFF during the fleet-transition
// window: the shipped v4.0.18 client sends no code, so requiring one is a
// total login outage. See the note above REQUIRE_HANDOFF_CODE in
// routes/auth.js, and google-oauth-handoff-transition.test.js for the
// behaviour that actually ships first.
process.env.GOOGLE_POLL_REQUIRE_CODE = 'true';
