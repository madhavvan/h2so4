...
export async function proxyRequest(endpoint: string, body: any): Promise<string> {
  ...
  const fire = () => fetch(`${API_BASE}/api/v1/ai${endpoint}`, {
    ...
    headers: {
      ...
      // Which client generation is calling. The session gate
      // (requireActiveSession in routes/ai.js) enforces only for clients
      // that can render its 428 — see the note there. Without this header
      // the server cannot tell a v4.0.18 install from a current one, and
      // enforcing blindly is how a server-side change breaks the fleet.
      'X-App-Version': licenseService.getAppVersion(),
    },
  ...
  const response = await withRetry(async () => {
    ...
    const res = await fetch(`${API_BASE}/api/v1/ai${endpoint}`, {
      ...
      headers: {
        ...
        // See the note on the non-streaming path above.
        'X-App-Version': licenseService.getAppVersion(),
      },
      body: JSON.stringify(body),
      signal,
